...
import { CalendarDays, LayoutDashboard, LogOut, Menu, Scissors, ShieldCheck, UserRound, Users } from "lucide-react";
...
export default function AdminShell({ adminName, children }: Props) {
  ...
  const sidebar = (
    <div className="flex h-full flex-col bg-gray-950 text-white">
        ...
        {links.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium ${active ? "bg-amber-600 text-white" : "text-gray-300 hover:bg-white/10 hover:text-white"}`}>
              <Icon size={19} />{hi ? ({ Dashboard: "डैशबोर्ड", Tailors: "दर्जी", Verification: "सत्यापन", Customers: "ग्राहक", Bookings: "बुकिंग" } as Record<string, string>)[item.label] : item.label}
            </Link>
          );
        })}
      </nav>
      <div className="border-t border-white/10 p-4">
        ...
        <button type="button" onClick={() => signOut({ callbackUrl: "/admin/login" })} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-red-300 hover:bg-red-500/15"><LogOut size={19} />{hi ? "लॉग आउट" : "Logout"}</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-100 lg:flex">
      ...
      {open && <div className="fixed inset-0 z-50 lg:hidden"><button aria-label={hi ? "एडमिन मेनू बंद करें" : "Close admin menu"} onClick={() => setOpen(false)} className="absolute inset-0 bg-black/50" /><aside className="relative h-full w-72">{sidebar}</aside></div>}
      <div className="min-w-0 flex-1 lg:ml-64">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-gray-200 bg-white px-4 sm:px-6 lg:px-8">
          ...
          <p className="font-semibold text-gray-900">{hi ? "प्रशासन" : "Administration"}</p>
